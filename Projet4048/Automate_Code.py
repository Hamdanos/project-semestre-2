...
def merge_graphs(filenames, output_filename):
    """
    Fusionne plusieurs graphes graphviz en un seul fichier.
    
    :param filenames: Liste des noms de fichiers des graphes à fusionner.
    :param output_filename: Nom du fichier de sortie pour le graphique combiné.
    """
    combined_dot = "digraph {\n"
    for filename in filenames:
        with open(filename, "r") as file:
            combined_dot += file.read()
    combined_dot += "}"
    with open(output_filename, "w") as file:
        file.write(combined_dot)
...
if __name__ == "__main__":
    ...
    if is_recognized:
        print(f"Le mot '{word}' est reconnu par l'automate.")
    else:
        print(f"Le mot '{word}' n'est pas reconnu par l'automate.")
